 ...
 */
public class AstarWAgentCost implements TWPathGenerator {
     ...
     */
    public AstarWAgentCost(TWEnvironment map, TWAgent agent, int maxSearchDistance) {
    ...
    public TWPath explorePath(int sx, int sy, int tx, int ty, int agentx, int agenty) {
      // easy first check, if the destination is blocked, we can't get there
      if (agent.getMemory().isCellBlocked(tx, ty)) {
          return null;
      }
          ...
          for (int x = -1; x < 2; x++) {
              for (int y = -1; y < 2; y++) {
                  ...
                  if (isValidLocation(sx, sy, xp, yp) && !agent.getMemory().isCellBlocked(xp, yp)) {
                      ...
                      // in the sorted open list
                      double nextStepCost = current.cost + getMovementCost(current.x, current.y, xp, yp) +
                        getAgentProximityCost(sx, sy, agentx, agenty);
                      Node neighbour = nodes[xp][yp];
                      neighbour.setVisited(true);
                      ...
                      if (!inOpenList(neighbour) && !(inClosedList(neighbour))) {
                          neighbour.cost = nextStepCost;
                          neighbour.heuristic = getHeuristicCost(xp, yp, tx, ty) + getAgentProximityCost(sx, sy, agentx, agenty);
                          maxDepth = Math.max(maxDepth, neighbour.setParent(current));
                          addToOpen(neighbour);
                      }
                  }
